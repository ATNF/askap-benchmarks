...
int main(int /*argc*/, char** /* argv*/)
{
    ...
    //
    // Run the MIC OpenMP version of the code
    //
    vector<float> micResidual(dirty.size());
    vector<float> micModel(dirty.size());
    zeroInit(micModel);
    {
        // Now we can do the timing for the MIC OpenMP CPU implementation
        cout << "+++++ Forward processing (MIC OpenMP) +++++" << endl;

        Stopwatch sw;
        sw.start();
        mic_deconvolve(&dirty[0], dim, &psf[0], psfDim, &micModel[0], &micResidual[0]);
        const double time = sw.stop();
    ...
    const bool modelDiff = compare(goldenModel, micModel);
    if (!modelDiff) {
        return 1;
    } else {
        cout << "Pass" << endl;
    ...
    const bool residualDiff = compare(goldenResidual, micResidual);
    if (!residualDiff) {
        return 1;
    } else {
        cout << "Pass" << endl;
