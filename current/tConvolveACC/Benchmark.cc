...
void Benchmark::runGridACC()
{
    gridKernelACC(C, grid2, gSize);
}
...
void Benchmark::runDegridACC()
{
    degridKernelACC(grid2, gSize, C, outdata2);
}
...
void Benchmark::gridKernelACC(const std::vector<Value>& C,
                              std::vector<Value>& grid,
                              const int gSize)
{

    // dereference to avoid touching the host within the device parallel area
    ...
    const int *d_iw = wPlane.data();
    const int *d_sSize = sSize.data();
    ...
    for (dind = 0; dind < d_size; ++dind) {
        ...
        for (suppv = 0; suppv < mySize; suppv++) {
...
// Perform degridding
void Benchmark::degridKernelACC(const std::vector<Value>& grid,
                                const int gSize,
                                const std::vector<Value>& C,
                                std::vector<Value>& data)
{
    ...
    for (dind = 0; dind < d_size; ++dind) {
        ...
        for (int suppv = 0; suppv < mySize; suppv++) {
            for (int suppu = 0; suppu < mySize; suppu++) {
                const Value cval = d_grid[gind+suppv*gSize+suppu] * d_C[cind+suppv*mySize+suppu];
                re = re + cval.real();
                im = im + cval.imag();
            }
        }
